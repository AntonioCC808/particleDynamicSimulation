...
public class Force implements pds.common.IForce {
	/**
	 * The state for which forces and hence accelerations are calculated. This value
	 * is initialized by the constructor.
	 */
	protected pds.common.State s;

	// Parameter sigma of the Lennard-Jones potential in reduced units
	public static double sigmaReduced = 1.0;

	/**
	 * This static method creates an object of a class that implements the IForce
	 * interface
	 * 
	 * @param s the reference to the state object that holds the system for which
	 *          the acceleration and potentials will be calculated.
	...
	/**
	 * This constructor receives the state for which the Force object will be
	 * created.
	 * 
	 * @param s the state for whose particles the accelerations will be calculated
	 *          when requested
	 */
	 ...
	 * corresponding accelerations in the current State. The operation returns the
	 * current potential energy of the system. The positions of every particle are
	 * taken from State, as well as the Neighbor list of each particle. For a
	 * particle, just the particles stated in the neighbor list are taken into
	 * account. The force acting over a particle because of the other ones has three
	 * components. First, it calculates the distance between each neighbor particle
	 * and the force is calculated using the equation for every particle against
	 * this one, therefore the total force in a component is the sum of all these
	 * pair forces. The force acting on a particle operates in the same way for the
	 * 3 components.
	 * 
	...
	/**
	 * This function returns the potential between two particles of the current
	 * experiment evaluated for a concrete distance given as input parameter.
	 * 
	 * @param distance value in sigma units that indicates the distance between the
	 *                 particles for which the potential is to be calculated
	 * @return value of the potential calculated for two particles of the current
	 *         experiment when they are at the distance given as invocation
	 *         argument.
	 */
	 ...
	 * describe the potential for the range of distance values observed in the
	 * current experiment. The number of points the pareameter of the input histogram resolution. 
	 * The objective of this method is to provide (review) graph which contains two list of numbers In the x coordinate
	 * we take as a first value, the minimum distance mind and as a maximum value
	 * the maximum distance maxd. The intermediate points are values separate
	 * (maxd-mind)/histogramResolution In the Y coordinate, the potential for each value
	 * of the distances is calculated using the method evalPotential
	 * 
	 * @return a graph that is a list of pairs of numbers (x,y). These values are of
	 *         the type double. x represents the distance in meters and y potentials
	 *         in energy units. The first and last points in the graph correspond to
	 *         the minimum and maximum distances observed among all particles in the
	 *         current experiment.
	 */
	...
	/**
	 * This method returns a graph in the form of a list of points (x,y) that
	 * describe the force for the range of distance values observed in the
	 * current experiment. The number of points the pareameter of the input histogram resolution. 
	 * The objective of this method is to provide (review) graph which contains two list of numbers In the x coordinate
	 * we take as a first value, the minimum distance mind and as a maximum value
	 * the maximum distance maxd. The intermediate points are values separate
	 ...
	 * 
	 * @return a graph that is a list of pairs of numbers (x,y). These values are of
	 *         the type double. x represents the distance in meters and y force in reduced units.
	 *         The first and last points in the graph correspond to
	 *         the minimum and maximum distances observed among all particles in the
	 *         current experiment.
	 */
	...
	/**
	 *This method compute the pair distribution function and store the data in a graph.
	 *The pair distribution function, defined in Eq. (2.94) of the Allen-Tildesley
	 *textbook, can be simply thought of as the number of atoms a distance r
	 *from a given atom compared with the number at the same distance in an 
	 *ideal gas at the same density.
	 * @return a graph that is a list of pairs of numbers (x,y). These values are of
	 *         the type double. x represents the distance in meters between two particles
	 *         and y force the frequency for that distance.
	 */
